...
public class CharImageLoader {

    private final StorageReference storageRef = FirebaseStorage.getInstance().getReference();
    private final Context context;

    public CharImageLoader(Context context) {
        this.context = context;
    }

    public void listFiles() {
        // replace listRef with your reference
        StorageReference listRef = storageRef.child("/dataset/api-mistress/assets/images/characters");
        listRef.listAll()
                            ...
                            prefix.listAll().addOnSuccessListener(new OnSuccessListener<ListResult>() {
                                ...
                                public void onSuccess(ListResult listResult) {
                                    ...
                                    for(StorageReference item : listResult.getItems()){
                                        if(item.getName().equals("icon-big")){
                                            String prefix_name = prefix.getName();
                                            File dir = new File(context.getFilesDir(), "Character_Prefixes");
                                            if (!dir.exists()) {
                                            ...
                                            Log.d("CreateFile", "Successfully downloaded: " + file.getAbsolutePath());
                                            try {
    ...
    public void downloadFile(String prefix,StorageReference fileRef,String Filename){
        ...
        File directory = new File(context.getFilesDir(), "character_images");
        if (!directory.exists()){
